...
# 修改这行：不要导入logger，而是用logging
# from detectron2.utils import logger  # 删除这行
...
class Trainer(DefaultTrainer):
    ...
    def build_model(cls, cfg):
        """
        重写build_model以正确配置DDP
        """
        model = build_model(cfg)
        # 修改这行：使用logging.getLogger
        logger.info("Model:\n{}".format(model))
        return model
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, distributed_rank=comm.get_rank(), name="seqrank")
    return cfg
